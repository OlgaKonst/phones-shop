...
class Page {
    constructor(options) {
        ...
        this._phoneCatalogue= new PhoneCatalogue( {
            elem: document.querySelector('[data-component = "phoneCatalogue"]'),
            phones: this._phones,
   ...
   /* _onPhoneSelected(phoneId) {
    ...
    _onPhoneSelected(e) {
        ...
        let phoneDetailes = this._getPhoneDetailes(phoneId);
        this._phoneViewer.show(phoneDetailes);
    ...
    //подменяем обращение к серверу
    _getPhones() {
        return phones.slice();// сделали точную копию массива в новый массив. ссылка не будет передоваться. фактически создаемкаждый раз новый массивб когда получаем данные из серевера
    }
    //подменяем обращение к серверу
    _getPhoneDetailes(phoneId) {
      return this._phones.filter(function(phone){
    ...
    _goBack(e) {
        ...
        if(!e.targen.hasAttribute('selector')) return;
        alert('back');
    }
}
var phones = [
